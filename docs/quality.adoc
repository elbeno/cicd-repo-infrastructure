...
Python code may be formatted using https://github.com/psf/black[black]. There are
...
NOTE: `black` is used for Python formatting if `INFRA_PYTHON_FORMATTER` is set
to `black`. By default, it is set to xref:quality.adoc#_ruff[`ruff`].
...
By default, Python code is formatted using https://docs.astral.sh/ruff/[ruff]. There are
two targets with the appropriate behavior:

- `check-ruff-format`
- `fix-ruff-format`

NOTE: Like `fix-clang-format`, `fix-ruff-format` won't change files that are
already changed in git's working tree.
...
`fix-clang-format`, `fix-cmake-format`, and `fix-ruff-format` targets.
