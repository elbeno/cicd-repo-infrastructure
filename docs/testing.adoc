...
NOTE: Python tests use several testing packages; these can be installed using (the preferred method) `uv`
or (the older method) `pip` with `requirements.txt` which is provided through the `INFRA_PROVIDE_PYTEST_REQS`
option.

Example `pyproject.toml` snippet showing dependency on this repo package:
[source,toml]
----
[project]
dependencies = [
...
`add_approval_test` adds a Python approval test using https://pytest.org[pytest] and
https://approvaltests.com[approvaltests].

NOTE: `add_approval_test` is a wrapper for `add_unit_test(${name} PYTEST ...)`;
all the same arguments are available. Approval tests are executed using
`pytest`.
