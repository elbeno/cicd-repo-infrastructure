...
When `ON`, this repository provides a `puppeteer_config.json` file in the docs
directory. This configures puppeteer to build documentation.

*`INFRA_PROVIDE_MERMAID_CONFIG`*

When `ON`, this repository provides a `mermaid.conf` file in the docs
directory. This configures mermaid to build diagrams.
